...
class TapirBackend:
    ...
    def _resolve_callable(self) -> Callable[..., Any]:
        ...
        if not callable(fn):
            raise TypeError(f"Tapir backend target is not callable: {self.backend_callable}")
        return fn

    def track(self, sequence: RGBDSequence) -> TrackBatch:
        device = torch.device(self.device if torch.cuda.is_available() else "cpu")
        video = sequence.rgb.to(device).unsqueeze(0)  # [1,T,3,H,W]
        fg_mask = sequence.fg_mask.to(device) if sequence.fg_mask is not None else None

        fn = self._resolve_callable()
        # Expect callable signature: fn(video, fg_mask=None) -> (tracks, visibility)
        # tracks: [1,T,N,2], visibility: [1,T,N]
